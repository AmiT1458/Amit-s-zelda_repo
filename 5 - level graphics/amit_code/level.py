...
from Upgrade_screen import Upgrade
...
class Level:
    def __init__(self):
        ...
        self.player_magic = MagicPlayer(self.animation_player)
                ...
                if collision_sprites:
                    for target_sprite in collision_sprites:
                        if target_sprite.sprite_type == 'grass':
                            ...
                            offset = pygame.Vector2(0,60)
                            for leaf in range(randint(3,6)):
                                self.animation_player.create_grass_particles(pos - offset, [self.visible_sprites])
                            target_sprite.kill()
                        else:
                            target_sprite.damage_taken(attack_sprite.sprite_type,self.player)
    ...
    def trigger_deaths_particles(self,pos,particle_type):
    ...
    # draw stuff to the screen and update
    def run(self):
        self.visible_sprites.custom_draw(self.player)
        self.ui.display(self.player)
        if self.game_paused:
            self.upgrade.upgrade_screen()
        else:
...
class YSortCameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self,player):
        ...
        #print(self.offset.x)
        #checking if the player sees the black screen(black)
        #if it is the case no more offsetting the screen with the vector : self.offset
        if self.offset.y <= 0:
            self.offset.y = 0

        if self.offset.y >= 2474:
            self.offset.y = 2474

        if self.offset.x >= 2368:
            self.offset.x = 2368
